...
class cfg(cfg_common, cfg_dataset_default, cfg_model_LightAD):
	def __init__(self):
		# super(cfg, self).__init__()
		cfg_common.__init__(self)
		cfg_dataset_default.__init__(self)
		cfg_model_LightAD.__init__(self)
		...
		self.test_per_epoch = self.epoch_full // 8
		...
		self.data.root = '/home/dly/Desktop/Dataset/mvtec'
		...
		self.data.train_transforms = [
			...
			dict(type='CenterCrop', size=(self.CenterCrop, self.CenterCrop)),
			dict(type='ToTensor'),
		...
		self.model_t.kwargs = dict(pretrained=False, checkpoint_path='model/pretrain/resnet18.bin',
								   strict=False, features_only=True, out_indices=[1, 2, 3])
		self.model_s = dict(
			dims=[256, 256, 128, 64],
		...
		self.model.name = 'lightad'
		self.model.kwargs = dict(pretrained=True, checkpoint_path='', strict=True, \
		...
		# ==> trainer
		self.trainer.name = 'LIGHTADTrainer'
		...
		self.loss.loss_terms = [
			dict(type='L2Loss', name='Loss', lam=5.0),
		]
