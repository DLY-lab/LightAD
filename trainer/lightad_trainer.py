...
class LIGHTADTrainer(BaseTrainer):
    def __init__(self, cfg):
        super(LIGHTADTrainer, self).__init__(cfg)
    ...
    def optimize_parameters(self):
        ...
        with self.amp_autocast():
            self.forward()
            loss_mse = self.loss_terms['Loss'](self.feats_t, self.feats_s)
        self.backward_term(loss_mse, self.optim)
        update_log_term(self.log_terms.get('Loss'), reduce_tensor(loss_mse, self.world_size).clone().detach().item(), 1,
                        self.master)
    ...
    def test(self):
        ...
        while batch_idx < test_length:
            ...
            self.forward()
            loss_mse = self.loss_terms['Loss'](self.feats_t, self.feats_s)
            update_log_term(self.log_terms.get('Loss'), reduce_tensor(loss_mse, self.world_size).clone().detach().item(),
                            1, self.master)
            # get anomaly maps
